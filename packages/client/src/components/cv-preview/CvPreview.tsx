...
import {aboutMeData, contactInfoData, educationData, projectsData, skillsData, workExperienceData} from "@c-v-a-i/common";
import {ContactInfoItem} from "../atoms/ContactInfoItem";

export const CvPreview = () => {
    return (
     <Box sx={{ gap: '40px', display: 'flex', flexDirection: 'column', margin: "0 auto", maxWidth: '1400px', padding: '0 80px' }} >
          ...
          <Grid item xs={4} md={4}>
            <Box display="flex" flexDirection="column" gap="16px" sx={{textAlign: 'end'}}>
              <Box display="flex" flexDirection="column" gap="8px">
                {Object.entries(contactInfoData).map(
                  ([name, value]) => <ContactInfoItem key={name} name={name} value={value} />
                )}
              </Box>
